...
// --- EMERGENCY BODY PARSER FALLBACK ---
app.use((req, res, next) => {
  if (req.method === 'PATCH' || req.method === 'POST') {
    ...
    req.on('end', () => {
      try {
        if (rawData && (!req.body || Object.keys(req.body).length === 0)) {
...
function authenticateToken(req, res, next) {
  ...
  jwt.verify(token, JWT_SECRET, (err, user) => {
    if (err) {
      console.error("🔍 JWT Verification Failed:", err.message); // <-- Add this line
...
app.post('/api/auth/signup', async (req, res) => {
  try {
    ...
    const newUser = await prisma.user.create({
      data: { fullName, universityId, faculty, email, phone, passwordHash, role: role || 'STUDENT' }
    });
